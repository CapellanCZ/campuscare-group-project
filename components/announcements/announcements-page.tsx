...
export function AnnouncementsPage({
  ...
  const cardsLevel = getAccessLevel(d, "announcements.cards")
    ...
    <div className="flex flex-col gap-4 p-4 md:p-6">
      ...
      <Card className="min-w-0 shadow-none dark:ring-0">
        <CardHeader className="flex flex-col gap-3 border-b sm:flex-row sm:items-center sm:justify-between">
          ...
          <Input
            className="sm:w-72"
            placeholder="Search title, audience, or message"
            value={query}
            onChange={(event) => setQuery(event.target.value)}
          />
        </CardHeader>
        <CardContent className="p-0">
          {showSkeleton ? <AnnouncementsTableSkeleton /> : null}
          ...
          {!showSkeleton && !hasRows ? (
            <Empty className="border-0">
              <EmptyHeader>
                ...
                <EmptyDescription>
                  {canManage
                    ? "Create the first clinic notice for your team."
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            ...
            <Button
              ...
              onClick={() =>
                setPage((current) => Math.min(list.totalPages, current + 1))
              }
            >
              Next
            </Button>
      ...
      <AnnouncementDetailSheet
        ...
        onPublish={() => selected && handlePublish(selected)}
      />
