...
import { IconDots, IconSend, IconChecklist } from "@tabler/icons-react";
				...
				<ul className="flex flex-col divide-y divide-border">
					{teammates.map((t) => (
						...
						>
							...
							<DropdownMenu>
								<DropdownMenuTrigger render={<Button aria-label={`Actions for ${t.name}`} size="icon-xs" variant="ghost" />}><IconDots
                                										/></DropdownMenuTrigger>
								<DropdownMenuContent align="end" className="min-w-52">
									...
									<DropdownMenuItem className="gap-2">
										<IconSend className="size-4 opacity-70" />
										Message
									</DropdownMenuItem>
									<DropdownMenuItem
										className="gap-2"
									...
									>
										<IconChecklist className="size-4 opacity-70" />
