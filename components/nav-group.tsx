				...
				{items.map((item) => (
					<Collapsible className="group/collapsible" defaultOpen={
                    							...
                    							item.subItems?.some((i) => !!i.isActive)
                    								...
                    								<>
                    									<CollapsibleTrigger render={<SidebarMenuButton isActive={item.isActive} />}>{item.icon}<span>{item.title}</span><IconChevronRight className="ml-auto transition-transform duration-200 group-data-[state=open]/collapsible:rotate-90" /></CollapsibleTrigger>
                    									<CollapsibleContent>
                    										<SidebarMenuSub>
