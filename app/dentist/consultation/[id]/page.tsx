...
async function resolveConsultationId(id: string): Promise<string | null> {
  ...
  const { data: byAppointment } = await supabase
    ...
    .maybeSingle()
  if (byAppointment?.id) {
    redirect(`/dentist/consultation/${byAppointment.id}`)
  }
  ...
  if (appointment?.id) {
    ...
    if ("id" in ensured) {
      redirect(`/dentist/consultation/${ensured.id}`)
    }
  }
...
async function Content({ id }: { id: string }) {
  ...
  const workspace = await loadConsultationWorkspace(consultationId, "dentist")
